...
public class GenericParameterExtractor
{
    ...
    /// This method is useful when: <br/>
    /// - 'definition' is a base type or an interface of another generic type,
    /// where the generic parameters are probably not in the same order as the target type. <br/>
    /// - 'definition' contains nested generic types that are defined with generic type parameters. <br/>
    /// </summary>
    ...
    public static void ExtractArguments(Type type, Type definition, Span<Type> arguments)
    {
        ...
        for (var index = 0; index < targetArguments.Length; ++index)
        {
            ...
            if (definitionParameter.IsGenericTypeParameter)
                ...
                ExtractArguments(targetArgument, definitionParameter, arguments);
        }
    }
}