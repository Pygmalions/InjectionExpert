...
internal static partial class TypeExtensions
{
    ...
    extension(Type self)
    {
        ...
        {
            if (target.IsGenericParameter || self.IsGenericParameter)
                return !strict || self.IsGenericParameter && target.IsGenericParameter;
            ...
            return selfArguments.Index()
                .All(argument => Matches(argument.Item, otherArguments[argument.Index], strict));
        }
        ...
        /// and then convert Nullable{ValueTuple{T1, T2}} to Nullable{T}.
        /// Partially erasing is not supported,
        /// means that this method will erase all generic arguments at the same time,
        /// therefore, Nullable{ValueTuple{int, ValueTuple{int, long}}}
        /// will be converted to Nullable{ValueTuple{T1, T2}},
        /// rather than
        /// Nullable{ValueTuple{int, ValueTuple{int, long}}}.
        /// </summary>
        /// <returns>Type whose generic arguments of the deepest nest are erased.</returns>
            ...
            [NotNullWhen(true)] out Type? matchedInterfaceType)
        {
            ...
            // Here, the matched interface type is probably an interface with the same name of the specified generic interface
            // and the same number of generic arguments.
            matchedInterfaceType = self.GetInterface(interfaceType.Name);
            if (matchedInterfaceType == null)
                return false;
            if (matchedInterfaceType.Matches(interfaceType, false))
                return true;
